...
var makeState = function(initial) {
  ...
  var runTimers = function(time) {
    ...
    timers = timers.filter(function(timer) {
      if (timer.time <= 1="" time)="" {="" fire.push(timer);="" return="" false;="" }="" else="" true;="" });="" fire.foreach(function(timer)="" timer.callback();="" };="" var="" self="{" current:="" initial,="" getmaxtime:="" function()="" maxtime;="" },="" init:="" checkpoints.push(util.clone(self.current));="" fork:="" i="prev(self.current.time);" while="" (checkpoints.length="" -=""> i)
        checkpoints.pop();
    ...
    seek: function(time) {
      if (time <= maxtime)="" {="" self.rewind(time);="" }="" else="" if="" (time=""> maxTime) {
        self.advance(time);
      }
    },
